...
function KitchenMonitor() {
  ...
  useEffect(() => {
    async function checkAuth() {
      try {
        ...
        console.warn("Staff page auth check warning:", err);
      }
    }
    checkAuth();
  }, []);
  ...
  const fetchSupabaseOrders = async () => {
    try {
      ...
      if (!dbOrdersError && dbOrders) {
        ...
        const savedLocalStr = localStorage.getItem("ran-lung-get-orders");
        const savedLocal: OrderHistory[] = savedLocalStr ? JSON.parse(savedLocalStr) : [];

        setOrders((prev) => {
          const baseList = prev.length > 0 ? prev : savedLocal;
          const combined = [...mappedOrders, ...baseList, ...savedLocal];

          const uniqueOrders: OrderHistory[] = [];
          ...
          localStorage.setItem("ran-lung-get-orders", JSON.stringify(uniqueOrders));
          return uniqueOrders;
        });
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("local_order_placed", handleLocalUpdate);
    window.addEventListener("local_supabase_realtime_change", handleLocalUpdate);
    const interval = setInterval(fetchSupabaseOrders, 1500);

    return () => {
      supabase.removeChannel(ordersCh);
      window.removeEventListener("local_order_placed", handleLocalUpdate);
      window.removeEventListener("local_supabase_realtime_change", handleLocalUpdate);
  ...
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if ((e.key === "ran-lung-get-orders" || e.key === "supabase_orders") && e.newValue) {
        try {
          fetchSupabaseOrders();
  ...
  const isWaitingStatus = (status: string) => status === "รอดำเนินการ" || status === "รอรับออเดอร์" || status === "pending";
  const isCookingStatus = (status: string) => status === "กำลังทำ" || status === "กำลังปรุง" || status === "preparing";
  const isReadyStatus = (status: string) => status === "พร้อมเสิร์ฟ" || status === "delivering" || status === "ready";
  const isCompletedStatus = (status: string) => status === "สำเร็จ" || status === "completed";
  const isCancelledStatus = (status: string) => status === "ยกเลิก" || status === "cancelled";
  ...
  const advanceOrderStatus = async (id: string) => {
    ...
    if (isWaitingStatus(targetOrder.status)) { nextStatus = "กำลังทำ"; dbStatus = "preparing"; }
    else if (isCookingStatus(targetOrder.status)) { nextStatus = "พร้อมเสิร์ฟ"; dbStatus = "delivering"; }
    else if (isReadyStatus(targetOrder.status)) { nextStatus = "สำเร็จ"; dbStatus = "completed"; }
  ...
  const regressOrderStatus = async (id: string) => {
    ...
    const targetOrder = orders.find((o) => o.id === id);
    if (!targetOrder) return;
    if (isCookingStatus(targetOrder.status)) { nextStatus = "รอดำเนินการ"; dbStatus = "pending"; }
    else if (isReadyStatus(targetOrder.status)) { nextStatus = "กำลังทำ"; dbStatus = "preparing"; }
    else if (isCompletedStatus(targetOrder.status)) { nextStatus = "พร้อมเสิร์ฟ"; dbStatus = "delivering"; }
    const nextList = orders.map((o) => (o.id === id ? { ...o, status: nextStatus } : o));
    setOrders(nextList);
    localStorage.setItem("ran-lung-get-orders", JSON.stringify(nextList));
  ...
  const clearCompletedOrders = () => {
    ...
    const nextList = orders.filter(o => !isCompletedStatus(o.status) && !isCancelledStatus(o.status));
    setOrders(nextList);
    localStorage.setItem("ran-lung-get-orders", JSON.stringify(nextList));
  };

  const stats = useMemo(() => {
    const active = orders.filter(o => !isCompletedStatus(o.status) && !isCancelledStatus(o.status));
    return {
      totalActive: active.length,
      waiting: orders.filter(o => isWaitingStatus(o.status)).length,
      cooking: orders.filter(o => isCookingStatus(o.status)).length,
  ...
  const ordersByStatus = useMemo(() => {
    ...
    return {
      waiting: list.filter(o => isWaitingStatus(o.status)),
      cooking: list.filter(o => isCookingStatus(o.status)),
      ready: list.filter(o => isReadyStatus(o.status)),
    };
  }, [orders, typeFilter]);

  const filteredOrders = useMemo(() => {
    const list = orders.filter(o => typeFilter === "all" || o.orderType === typeFilter);
    if (statusFilter === "active") return list.filter(o => !isCompletedStatus(o.status) && !isCancelledStatus(o.status));
    if (statusFilter === "รอดำเนินการ") return list.filter(o => isWaitingStatus(o.status));
    if (statusFilter === "กำลังทำ") return list.filter(o => isCookingStatus(o.status));
    if (statusFilter === "พร้อมเสิร์ฟ") return list.filter(o => isReadyStatus(o.status));
    if (statusFilter === "สำเร็จ") return list.filter(o => isCompletedStatus(o.status));
    if (statusFilter === "ยกเลิก") return list.filter(o => isCancelledStatus(o.status));
      ...
      <div className="space-y-1.5">
        {order.items.map((i, idx) => (
          <div key={idx} className="flex justify-between items-center text-xs">
            <span className="font-semibold text-slate-700">{tMenu(i.name, "name")}</span>
    ...
    <div className="bg-slate-50 border border-slate-100 rounded-xl p-3 flex justify-between items-center text-xs">
      <div>
        ...
        <p className="text-[11px] text-slate-500 mt-0.5 truncate max-w-sm">
          {order.items.map(i => `${tMenu(i.name, "name")} x${i.qty}`).join(", ")}
        </p>
      </div>
